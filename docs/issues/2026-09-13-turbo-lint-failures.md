...
Status: **fixed 2026-09-17** on `upgrade/expo-sdk-58-beta`; `turbo run lint`
is 19/19 and `pnpm lint` exits 0

## Resolution (2026-09-17)

All four in the suggested order, plus the `--continue` question at the end of
this file, which is now answered yes in both places that run turbo from code:

1. **`@acme/art`** — `packages/art/eslint.config.mjs`, the two-line
   `baseConfig()` every other package uses.
2. **`@acme/avatar`** — `.probe/**` added to the shared ignore list in
   `packages/config/eslint/base.mjs` rather than to avatar's own config: the
   directory is generated by `tooling/icon-atlas`-style tooling
   (`tools/shader-probe/run.mjs`) and gitignored, and ESLint does not read
   `.gitignore`. That one entry cleared 2,685 of the run's errors, all of them
   `no-var` / `no-undef` inside a bundled three.js build.
3. **`mobile`** — the rule-scoped ignore, narrower than the tsconfig exclude: it
   turns off `import/no-unresolved` and nothing else, only under
   `components/splash/redraw/**`. The two react-hooks errors in that directory
   were fixed rather than silenced (`tagline.value =` → `tagline.set(…)`, and a
   render-phase ref write moved into its own effect).
4. **`@acme/ui`** — `CollapsiblePane.tsx` now drops `grown` during render
   instead of from an effect. A pure derivation does not work here: the stored
   bit has to be cleared, or the next open re-applies `grow` on the first frame
   and the pane snaps to the window's leftover width, which is the bug the
   component exists to prevent.

Two things this file did not reach, both of which also blocked `pnpm lint`
before `turbo run lint` was ever invoked — the script runs twenty tooling gates
first:

- `tooling/check-references.mjs` walked into `packages/ui/node_modules` and
  demanded Mobbin citations from expo-router's components (32 of its 35
  offenders). It now carries the same `SKIP_DIR` regex as
  `check-role-accent.mjs`. The three real surfaces left were dealt with on their
  merits: two got references pulled from the Mobbin MCP, and `ViroIcon.tsx` is
  now exempt under a behavioural leaf-primitive rule — one childless element and
  nothing else draws a single thing and lays out nothing.
- `apps/web`'s lint read its own build output. `public/pdfjs/**` and
  `public/natalie/**` are declared outputs of `web#build` and are gitignored;
  `public/**` is now in the app's `globalIgnores`.
