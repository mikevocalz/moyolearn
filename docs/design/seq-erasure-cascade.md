...
| Educational store (doc 12 §4) | `packages/payload/migrations/edu_schema.sql` : `edu.transcripts`, `edu.knowledge_graph`, domains `opaque_id`/`fact_key`/`tag`/`label` · `packages/payload/migrations/edu_embeddings.sql` : `edu.embeddings` (pgvector, HNSW) | real, applied as migrations `edu_schema` and `edu_embeddings_pgvector`; **no production writer yet** — the tutoring path still writes `payload.session_transcripts` |
| Edu cascade leg | `apps/web/lib/edu.repository.ts` : `loadExpiredEduTranscripts`, `loadEduFactsDerivedFrom`, `eraseEduFacts`, `updateEduFactProvenance`, `deleteEduTranscripts` · `apps/web/lib/edu.client.ts` : `withEdu` | real, called by the same `POST /api/retention/sweep` on the same cutoff |
| Vector inheritance | `edu.embeddings.transcript_id` FK `ON DELETE CASCADE` → `edu.transcripts` | real; doc 19 §5.5 is enforced by the constraint, so no sweep has to remember it |
| Store-separation gate | `tooling/check-store-separation.mjs` (runs in `pnpm lint`) | real; fails if anything outside a `*.repository.ts` imports the edu client or writes `edu` SQL, or if a `packages/app` feature/service holds a raw driver |
| Edu cascade proof against real rows | `packages/payload/src/retention/erasure.integration.test.mjs` : *the sweep spans the edu schema — transcripts, derived facts and their vectors* | real; skipped without a `DATABASE_URL` |
