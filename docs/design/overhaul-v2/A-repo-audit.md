...
- Role accents CONFIRMED per doc 36 — derived in OKLCH, **shipped as hex** (RN can't evaluate `oklch()`): learner `#FFDB33`, guardian `#95EBFF`, tutor `#EDD4FF`, teacher `#FFC7B2`, org `#FFD7A5`, school `#BFF5C8`, district `#83EFF5`, each with pre-resolved `-underlay` rgba(0.24); ≥14:1 parity bar; `accentRoles` drives `.role-*` scopes; admin mints nothing; `tooling/check-role-accent.mjs` enforces the slot allowlist.
