...
} from './conform/driver.ts';
export { sha256, sha256Float32, sha256Hex } from './crypto/sha256.ts';

// The stage: rig, IBL, shadows, tone mapping, post chain.
export {
...
// Materials — three.js node materials, so they live here rather than on the
// three-free barrel.
export {
...
export {
  ...
  type HairDebugMode,
  type HairMaterial,
  type HairMaterialOptions,
  type HairUniforms,
} from './materials/hair.ts';
export {
  DENIM_PHASE_CHANNEL,
  ...
  seedPhase,
  type DenimMaterial,
  type DenimMaterialOptions,
  type DenimRegion,
  type DenimUniforms,
} from './materials/denim.ts';
export {
  CAVITY_ATTRIBUTE,
  CAVITY_FLOOR,
...
export {
  ...
  createBrowMaterial,
  type BrowMaterial,
  type BrowMaterialOptions,
} from './materials/brow.ts';

// Lashes — geometry plus the material for the offline-baked strand texture.
export {
  LOWER_LENGTH,
  ROWS,
  TILE_METRES,
...
// The GPU head path (doc 22 §4 row 14). Gated — `canUseComputeHead` decides,
// and a `false` verdict means the CPU path in `@acme/avatar/gnm` stays in use.
export {
  ...
  type ComputeGate,
  type HeadCompute,
  type HeadComputeRequirement,
  type HeadComputeSource,
} from './compute/head.ts';

// The CDN capability manager. Not three-dependent, but it is the thing that
// hands the loaders their file URIs, so it belongs beside them.
export {
  REQUIRED_RN_GLOBALS,
...
// Dev/QA only — the product camera is authored (doc 23), never orbited.
export {
...
// The golden gate. `golden.ts` is the DEVICE half and imports nothing Hermes
// lacks; `png.ts`/`pixel-diff.ts` are the HOST half (node:zlib) and are reached
// through `tools/golden-compare.ts`, not through this barrel.
export {
  GOLDEN_BUDGET,
  GOLDEN_CAMERAS,
  GOLDEN_FRAME_MS,
  ...
  summarise,
  type CameraVerdict,
  type CaptureInvariants,
  type CaptureOptions,
  type GoldenCamera,
  type GoldenFrame,
  type GoldenReport,
  type GoldenTarget,
} from './testing/golden.ts';
export {
  GOLDEN_HEIGHT,
  GOLDEN_WIDTH,
...
} from './testing/offscreen-target.ts';
