...
import { attribute, materialOpacity, smoothstep } from 'three/tsl';
import { Color, DoubleSide } from 'three';

/** Normalised distance along a strand: 0 at the root, 1 at the tip. */
export const BROW_TIP_ATTRIBUTE = 'aTip';

/** Where the fade begins and how much alpha it removes. Reference values. */
export const BROW_FADE_START = 0.55;
export const BROW_FADE_AMOUNT = 0.85;

type Float = Node<'float'>;
/**
 * `attribute()` is declared as `AttributeNode<string>` and carries no math
 * operators. Narrowed once at the boundary — same upstream gap as `eyes.ts`,
 * `hair.ts` and `mouth.ts`.
 */
 ...
 */
export function browOpacityNode(): Float {
  const tip = asFloat(attribute(BROW_TIP_ATTRIBUTE, 'float'));
  const fade = smoothstep(BROW_FADE_START, 1.0, tip).mul(BROW_FADE_AMOUNT);
  return asFloat(materialOpacity.mul(fade.oneMinus()));
}
...
export function createBrowMaterial(options: BrowMaterialOptions = {}): BrowMaterial {
  const material = new MeshStandardNodeMaterial({
    name: 'brow-strands',
    color: options.color ?? new Color(0x1d1410),
    roughness: 0.8,
    metalness: 0,
    // Hair takes very little from the environment; leaving this at 1 makes the
    // brows read as wet plastic under the RoomEnvironment PMREM.
    envMapIntensity: 0.25,
    transparent: true,
    // Ribbons are one-sided geometry viewed from both sides as the head turns.
    side: DoubleSide,
  });

  material.opacityNode = browOpacityNode();
