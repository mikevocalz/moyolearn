...
export async function readDocumentUsing(
  ...
  pdf?: (bytes: Uint8Array) => Promise<DocumentReading>,
): Promise<DocumentReading> {
  try {
    const bytes = await load(uri);
    if (bytes.length > MAX_DOCUMENT_BYTES) return { text: '', reason: 'failed' };
    const isPdf = mimeType === 'application/pdf' || String.fromCharCode(...bytes.subarray(0, 4)) === '%PDF';
    if (isPdf && pdf) return await pdf(bytes);
