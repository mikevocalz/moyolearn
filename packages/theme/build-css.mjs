...
const SITE_SCOPE = `@layer base {
  ...
  /*
    ...
    catch. Declared as real classes instead.
  */
}

/*
  UNLAYERED, AND DELIBERATELY DOUBLED, because these lost every argument they
  were in.

  react-native-web resets \`border-width: 0\` on every element, and that reset is
  UNLAYERED. An unlayered declaration beats every layered one no matter how
  specific — layer order is consulted before specificity. While these classes
  lived in \`@layer base\` they therefore did nothing at all: measured on the
  built marketing page, 65 of 71 elements carrying a \`border-moyo-*\` class
  computed to \`border-width: 0px\`. Every neubrutalist frame on the site — the
  desk cells, the crop marks, the nav rule, the tutor-room plate — was absent,
  and nothing failed because a missing border is not an error. Only \`<button>\`
  survived, because the kit's Button sets its own width inline.

  Unlayered alone is not enough: react-native-css injects its reset into the
  document at runtime, so it lands after this stylesheet and would win on source
  order at equal specificity. The doubled class is what makes the outcome
  independent of injection order.

  The cost, stated: a call-site \`border-4\` can no longer override these. That
  is acceptable here — a raw border utility is a hardcoded value, which the UI
  law bans anyway. Override by choosing a different token, not a bare number.
*/
.border-moyo-hair.border-moyo-hair { border-width: var(--moyo-border-w-hair); }
.border-moyo-rule.border-moyo-rule { border-width: var(--moyo-border-w-rule); }
.border-moyo-slab.border-moyo-slab { border-width: var(--moyo-border-w-slab); }
`;
