...
import { createAdaptivePanesStore } from './store.ts';
...
describe('paneVisibility', () => {
  ...
  it('gives the three-pane shape sidebar + supplementary + inspector only at large', () => {
    const xl = paneVisibility('large', 2);
    assert.deepEqual(xl, {
      primary: true,
      supplementary: true,
      inspector: true,
...
describe('selection', () => {
  it('starts empty and is set only by an explicit select', () => {
    const store = createAdaptivePanesStore();
    assert.equal(store.getState().selectedId, null);
    store.getState().select('report-1');
    assert.equal(store.getState().selectedId, 'report-1');
    store.getState().select(null);
    assert.equal(store.getState().selectedId, null);
  });

  it('survives the fold: column stepping and resizing never clear it (doc 37 §3.2)', () => {
    const store = createAdaptivePanesStore();
    store.getState().select('report-1');

    // compact -> expanded -> compact travel, plus a resize mid-way.
    store.getState().setColumn('secondary');
    store.getState().setPrimaryWidth(320);
    store.getState().setColumn('primary');
    store.getState().resetPrimaryWidth();
  ...
  it('is scoped per instance — two hosts never share a selection', () => {
    const notes = createAdaptivePanesStore();
    const reports = createAdaptivePanesStore();
    notes.getState().select('draft-9');
