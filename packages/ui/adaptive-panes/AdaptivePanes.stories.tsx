// AdaptivePanes — the host with a supplied detail pane, plus the pane
...
/*
  The HOST is storyable now: the `detail` escape hatch renders supplied content
  instead of requiring expo-router's <Slot/>, so no router context is needed —
  the same seam that lets apps/web mount it. The pane-composition stories below
  it exercise the chrome the host arranges.
*/
const meta = { title: 'Interaction/AdaptivePanes' } satisfies Meta;
export default meta;
...
/**
 * The list pane of the host story: rows write to the host's own scoped store
 * via `useAdaptivePaneSelection` — no story-local state, the same wiring the
 * app's mounts use.
 */
function HostList() {
...
/** The detail pane: reads the same scoped selection the list writes. */
function HostDetail() {
  const { selectedId, select } = useAdaptivePaneSelection();
  ...
  if (!selected) {
    return (
      <View className="flex-1 items-center justify-center p-inset">
        <Text className="text-caption text-text-muted">Select a session note</Text>
      </View>
    );
  }
  return (
    <View className="flex-1">
      <DetailNavbar title={selected.title} onDismiss={() => select?.(null)} />
...
/**
 * THE HOST — `AdaptivePanes` itself, storyable through the `detail` escape
 * hatch: the primary column is the queue list, the supplied `detail` content
 * renders where the router's Slot would on device. Selection lives in the
 * host's per-instance store, so it survives the viewport crossing a width
 * class (resize the window: compact collapses to the list, expanded tiles
 * list beside detail — the selection is still there).
 */
export const HostTwoPaneWithSelection: Story = {
  render: function Host() {
    return (
      <View className="min-h-96 w-full flex-1 overflow-hidden border-2 border-border">
        <AdaptivePanes detail={<HostDetail />}>
...
export const ThreePaneAdaptive: Story = {
  render: function ThreePane() {
    ...
    const showSupplementary = sizeClass === 'large';
    const showPrimary = sizeClass === 'large' || sizeClass === 'expanded';
